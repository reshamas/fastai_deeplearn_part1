...
- We could then just call `Learner` to turn that into a learner but if we call RNN learner..., RNN learner is a learner.  It defines cross-entropy as the default criteria.  In thise case `class RNN_Learner`, we're overwriting that anyway so that's not what we care about.  But, it does add in these `save_encoder` and `load_encoder` things that can be handy sometimes.  So we could have, in this case, we really could have just said learner.  But, `RNN_Learner` also works.  So, here's how we turn our PyTorch module into a fastai model
